//
//  DynamicHeightCellViewController.m
//  AppDevKit
//
...
@interface DynamicHeightCellViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, ADKModalMaskViewDelegate>
...
@implementation DynamicHeightCellViewController
...
- (void)setupView
{
    self.title = @"Dynamic Height Cell List View";
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell) {
        ...
        avengersCell.descriptionLabel.numberOfLines = (!self.dynamicMode) ? 2 : 0;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (!self.dynamicMode) {
        ...
        // CGFloat targetWidth = CGRectGetWidth(collectionView.bounds);
        cellSize = [[ADKNibSizeCalculator sharedInstance] sizeForNibNamed:CellCollectionViewCellIdentifier
                                                             withStyle:ADKNibFixedHeightScaling];
        NSDictionary *dict = self.flagArray[indexPath.row];
        SampleVCollectionViewCell *avengersCell = (SampleVCollectionViewCell *)[[ADKNibCacheManager sharedInstance] instanceForNibNamed:CellCollectionViewCellIdentifier];
        avengersCell.imageView.image = [UIImage imageNamed:dict[@"photo"]];
        avengersCell.titleLabel.text = dict[@"title"];
        avengersCell.descriptionLabel.lineBreakMode = NSLineBreakByTruncatingTail;
        avengersCell.descriptionLabel.numberOfLines = 0;
        avengersCell.descriptionLabel.text = dict[@"desc"];

        cellSize = [[ADKCellDynamicSizeCalculator sharedInstance] sizeForDynamicHeightCellInstance:avengersCell
