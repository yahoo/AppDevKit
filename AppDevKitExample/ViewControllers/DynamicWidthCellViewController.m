//
//  DynamicWidthCellViewController.m
//  AppDevKit
//
...
#import "SampleHCollectionViewCell.h"
#import "ModalViewController.h"

static NSString * const CellCollectionViewCellIdentifier = @"SampleHCollectionViewCell";

@interface DynamicWidthCellViewController () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, ADKModalMaskViewDelegate>
...
@property (assign, nonatomic) BOOL dynamicMode;

@end

@implementation DynamicWidthCellViewController
...
- (void)setupView
{
    self.title = @"Dynamic Width Cell List View";
    ...
    UIBarButtonItem *supportBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemRefresh
                                                                                          target:self
                                                                                          action:@selector(supportSwitchHandler)];
    self.navigationItem.rightBarButtonItem = supportBarButtonItem;
}


#pragma mark - Event handlers

- (void)supportSwitchHandler
{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (cell) {
        ...
        SampleHCollectionViewCell *avengersCell = (SampleHCollectionViewCell *)cell;
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    CGSize cellSize;
    if (!self.dynamicMode) {
        cellSize = [[ADKNibSizeCalculator sharedInstance] sizeForNibNamed:CellCollectionViewCellIdentifier
                                                             withStyle:ADKNibOriginalSize];
    } else {
        cellSize = [[ADKNibSizeCalculator sharedInstance] sizeForNibNamed:CellCollectionViewCellIdentifier
                                                             withStyle:ADKNibOriginalSize];
        NSDictionary *dict = self.flagArray[indexPath.row];
        SampleHCollectionViewCell *avengersCell = (SampleHCollectionViewCell *)[[ADKNibCacheManager sharedInstance] instanceForNibNamed:CellCollectionViewCellIdentifier];
        avengersCell.imageView.image = [UIImage imageNamed:dict[@"photo"]];
        avengersCell.titleLabel.text = dict[@"title"];

        cellSize = [[ADKCellDynamicSizeCalculator sharedInstance] sizeForDynamicWidthCellInstance:avengersCell
